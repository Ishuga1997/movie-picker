...
import type { Movie, YearFilter } from '../../types'
...
export default function WatchlistPage() {
  ...
  useEffect(() => {
    fetch('/api/watchlist')
      ...
      .then(({ movies }: { movies: Movie[] }) => setMovies(movies))
      .catch(() => {})
  ...
  const removeFromWatchlist = (movieId: number) => {
    ...
    fetch('/api/watchlist', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tmdbId: movieId }) }).catch(() => {})
  }
  ...
  if (loading) return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-zinc-100 mb-1">Watchlist</h1>
      </div>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-zinc-100 mb-1">Watchlist</h1>
        <p className="text-zinc-500 text-sm">{movies.length} title{movies.length !== 1 ? 's' : ''} saved for later</p>
      </div>
      ...
      {movies.length === 0 ? (
        <div className="text-center py-24 text-zinc-600">
          ...
          <p className="text-sm">Tap &ldquo;Watch later&rdquo; on any movie to save it here</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-24 text-zinc-600">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {filtered.map((movie) => (
            <MovieCard
              ...
              movie={movie}
              isWatchlisted
              onToggleWatchlist={() => removeFromWatchlist(movie.id)}
            />
          ))}
        </div>
      )}
